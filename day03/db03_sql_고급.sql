...
SELECT SUBSTR('Hello MySQL', 7, 2);  -- 연습이 많이 필요함
...
-- 4-3 2014년 7월 7일에 주문받은 도서의 주문번호, 주문일, 고객번호, 도서번호를 모두 나타내시오.
-- 단, 주문일은 %Y-%m-%d 형태로 표시한다.
-- %Y = 년도전체, %y = 년도 뒤2자리, %M = July(월이름), %b = Jul(월약어) %m = 07(월숫자)
-- %d = 일, %H = 16시, %h = 04시(오후도), %W = Monday, %w = 1(요일은 일요일 0)
-- %p = AM/PM
SELECT orderid AS '주문번호'
