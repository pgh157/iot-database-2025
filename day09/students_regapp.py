...
def showDatas():
    ...
    for i,  (std_id, std_name, std_mobile, std_regyear) in enumerate(data): # enumerate - 인덱스와 아이템 동시접근
...
def getData(event):
    ...
    ## 엔트리 위젯에 각각 채워넣기
    ent1.config(state= 'normal')     # 데이터 들어 갈 수 있게 해주고
...
myFont = Font(family='NanumGotich', size=10)  # 이후에 화면 위젯에 저장할 동일폰트 생성
