...
- 데이터베이스 연동 프로그램
    - PyMySQL 모듈사용
    - 파이썬 DB연동 콘솔 : [노브북](./day05/db05_파이썬_DB연동.ipynb)
...
- 정규화 : [SQL](./day06/db03_정규화_이상현상.sql)
    ...
    - 함수 종속성 - 하나의 속성(A)이 다른 속성(B)을 결정지으면 A는 B의 결정자. A → B. A가 기본키가 됨
        ...
        - BCNF해당하지않는 경우 - A → C, B → C 일 경우 C → B 경우가 발생
...
- 트랜잭션 : [SQL](./day06/db05_트랜잭션2.sql)
    ...
    - 특징
        ...
        - D(지속성) - Durabiliy, 트랜잭션 후에 저장된 데이터는 무한히 값이 유지되어야 함.
    - 동시성 제어 : [SQL1](./day06/db09_동시성제어1.sql) / [SQL](./day06/db10_동시성제어2.sql) 
        - 락
...
- 데이터모델링 실습
    - 병원업무관리 ERD
        - 요구사항으로 개체와 관계를 정립. 개체에 속하는 속성들, 실별자 결정
        - 테이블 명세서 작성(엑셀, 워드)
...
- tkinter DB연동 GUI앱 개발 : [Python](./day09/students_regapp.py)
    ...
    2. madang DB에 students 테이블 생성
        ...
        create table students(
            std_id integer primary key auto_increment,  -- auto_increment는 MySQL옵션.
