...
SELECT SUM(saleprice) AS 총판매액   
	   ...
	   ,MIN(saleprice) AS 최저 
	   ,MAX(saleprice) AS 최고
       , STD(saleprice) AS StandardV
  FROM Orders;

-- 그룹화 GROUP BY 키워드 사용
-- 3-19 고객별로 주문한 도서의 총수량과 총판매액을 구하시오.
-- GOUP BY를 사용하면 반드시 집계함수 및 GROUP BY에 포함된 컬럼이 SELECT 안에 포함시켜야 함.
