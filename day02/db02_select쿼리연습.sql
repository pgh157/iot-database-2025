...
-- > 미만 < 초과 >= 이항 <= 이상 <> 같지않다 != 같지않다 = 같다 (프로그래밍언어와 차이)
SELECT *
  FROM Book
  WHERE price < 20000;
  
-- 3-5 : 가격이 10000원 이상 20000원이하인 도서를 검색하시오.
SELECT *
  FROM Book
...
SELECT *
 ...
 SELECT *
  FROM Book
 WHERE publisher = '굿스포츠' OR publisher = '한미디어';
...
-- 3-14 : 도서를 가격을 내림차순으로 검색하시오. 가격이 같다면 출판사를 오름차순으로 출력하시오.
SELECT *
 FROM Book
